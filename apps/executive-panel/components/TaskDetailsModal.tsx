// apps/executive-panel/components/TaskDetailsModal.tsx
"use client";
...
export default function TaskDetailsModal({ open, onClose, initial, onSave }: Props) {
  const [title, setTitle] = useState(initial?.title ?? "");
  const [owner, setOwner] = useState(initial?.owner ?? "");
  const [due, setDue] = useState(initial?.due ?? "");
  const [columnIndex, setColumnIndex] = useState(initial?.columnIndex ?? 0);
  ...
  const ready = title.trim().length > 0 && owner.trim().length > 0 && due.trim().length > 0;
    ...
    >
      <div className="card relative max-w-lg w-full max-h-[90vh] flex flex-col border border-gold/30 shadow-xl shadow-gold/10 bg-[#0D1220] overflow-hidden">
        ...
        <div className="flex-shrink-0 p-6 pb-4 border-b border-white/5 mb-4">
          ...
          <h2 className="section-title mb-1">Detalhes do Card</h2>
          <p className="text-pure/70 text-sm">Descrição e anexos desta tarefa</p>
        </div>
              ...
              <ul className="space-y-1">
                {attachments.map((att) => (
                  <li key={att.id} className="flex items-center justify-between text-sm text-pure/70">
                    <a href={att.url} target="_blank" rel="noreferrer" className="underline hover:text-gold">{att.name}</a>
                    <button
